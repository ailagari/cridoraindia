...
/** Vite --mode capacitor only reads .env.capacitor* — bake API URL for FCM + bell polling. */
const capLocalPath = path.join(root, '..', '.env.capacitor.local')
const capLocalLines = [
  '# Auto-generated by scripts/check-android-env.mjs',
  `VITE_API_BASE_URL=${apiBase}`,
]
const fcmMatch = envText.match(/^VITE_FCM_ENABLED=(.+)$/m)
if (fcmMatch) {
  capLocalLines.push(`VITE_FCM_ENABLED=${fcmMatch[1].trim()}`)
}
fs.writeFileSync(capLocalPath, `${capLocalLines.join('\n')}\n`)
console.log('[android:check] Wrote .env.capacitor.local (API URL baked into APK)')
